...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        self.instance_images_path = [x for x in self.instance_images_path if ('.png' in str(x) or '.jpeg' in str(x))]
...
class SimpleWrapper(torch.nn.Module):
  def __init__(self):
    super().__init__()
    layers = torch.nnn.ModuleList([torch.nn.Linear(768, 768) for _ in range(77)])
